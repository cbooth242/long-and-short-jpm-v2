...
import { ResponsiveContainer, BarChart, LineChart, AreaChart, Bar, Line, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend } from 'recharts';
...
const VisualChartContent = ({ c_content, templateName }) => {
  ...
  const generateChart = async () => {
    ...
    try {
      const allText = [c_content.title, ...(c_content.sections||[]).map(s => s.content)].filter(Boolean).join(' ');
      const resp = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01', 'anthropic-dangerous-direct-browser-access': 'true' },
        body: JSON.stringify({
          model: 'claude-haiku-4-5-20251001', max_tokens: 800,
          system: 'Extract or infer the most compelling data story from this investment content. Return ONLY valid JSON, no markdown.',
          messages: [{ role: 'user', content: `Read this investment piece and extract the most chart-worthy data or create plausible illustrative data based on the themes. Return JSON:\n{"title":"chart title","subtitle":"what this shows","type":"bar|line|area","xLabel":"x axis label","yLabel":"y axis label","data":[{"label":"label","value":number,"value2":number_optional}],"insight":"one sentence key insight from this chart","downloadable":true}\n\nContent: ${allText.slice(0, 1500)}` }]
        })
      });
      ...
      {chartData && Array.isArray(chartData.data) && chartData.data.length > 0 && (
        <div>
          <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: 24, marginBottom: 16 }}>
            ...
            {/* Chart area */}
            <div style={{ height: 240, display: 'flex', alignItems: 'flex-end', gap: 8, padding: '0 4px', borderBottom: '2px solid #E5E7EB', borderLeft: '2px solid #E5E7EB', position: 'relative' }}>
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
      <div style={{ padding: 16 }}>
        ...
        <div style={{ marginBottom: 14, padding: '8px 12px', background: '#FFF9ED', borderLeft: '3px solid ' + c.gold, borderRadius: '0 6px 6px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 10, color: c.slate }}>
            <strong style={{ color: c.gold }}>Ideas & Insights</strong> — 1–2 pages. Planning considerations and investment trends. Hard cap <strong>2,000 words</strong>. Include charts to support the story.
          </div>
          <div style={{ fontSize: 11, fontWeight: 700, color: warnOver ? c.neg : wordCount > 1600 ? '#E08A00' : wordCount > 0 ? c.pos : c.slate, marginLeft: 12, flexShrink: 0 }}>
            {wordCount}w {warnOver ? '⚠ Over 2,000w' : ''}
          </div>
        </div>
...
const ContentCircle = ({ currentContent, libraryItems, onOpen }) => {
  ...
  const scored = libraryItems
    ...
    .map(item => {
      ...
      const overlap = currentTags.filter(t => itemTags.includes(t)).length;
      score += overlap * 3;
      // Title keyword overlap
      const itemTitle = (item.title || '').toLowerCase();
      currentWords.forEach(w => { if (itemTitle.includes(w)) score += 1; });
      // Recency bonus
      const daysSince = (Date.now() - new Date(item.publishedDate || item.createdDate).getTime()) / 86400000;
      if (daysSince < 7) score += 2;
      else if (daysSince < 30) score += 1;
      // High score bonus
      if ((item.qScore || 0) >= 4) score += 2;
  ...
  return (
    <div style={{ padding:'24px 20px', maxWidth:900, margin:'0 auto' }}>
      ...
      {view === 'calendar' ? (
        ...
        past.map((e, i) => <EventCard key={i} event={e} isPast={true} />)
      )}
    </div>
  );
};
...
${sectionsText}`;
  ...
  const createNewContent = (templateId) => {
    ...
    setTemplateContents(prev => ({
      ...prev,
      [templateId]: blankTemplates[templateId] || { title: '' }
    }));
  ...
  const generateWhatsMoving = async () => {
    ...
    try {
      ...
      if (useSearch) {
        ...
        let msgs = [{ role: 'user', content: msg }];
        for (let i = 0; i < 3; i++) {
          const chunk = await callClaude({ model: 'claude-haiku-4-5-20251001', max_tokens: 1000, tools: TOOLS, messages: msgs });
          // callClaude returns text directly
          raw += chunk;
          break;
          break; // single call — tool use not supported via SSE in this context
        }
      } else {
      ...
      let finalParsed = { ...keep, ...parsed, ...(isDMU ? { title: currentContent.title } : {}) };
  ...
  return (
    <div style={{ minHeight: '100vh', background: c.ivory, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ padding: '8px 20px', background: '#fff', borderBottom: `1px solid ${c.pearl}`, display: 'flex', gap: 24 }}>
        {['Content Library', 'Editor', 'Workflow', 'Translations', 'Taxonomy', 'Data', 'Excel', 'Codebase', 'Forward Look', 'About'].map((tab, i) => {
          const id = ['content', 'editor', 'workflow', 'translations', 'taxonomy', 'data', 'excel', 'codebase', 'forwardlook', 'about'][i];
          return (
            <button 
              key={id} 
      ...
      <div style={{ padding: 20 }}>
        ...
        {activeTab === 'forwardlook' && <ForwardLookTab />}
